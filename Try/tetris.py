...
def main(player):
    ...
    global LINESFONT, NAME, WINNERFONT, enemy, sounds, level
    ...
    while enemy.getName() == '':
        ...
        print("Recebendo dados do adversário:", receive, '\n\n')
    ...
    # Loop do jogo
    while wins != 3 and enemy.getWins() != 3:
        random.seed()
        # Roda o jogo
        winner, wins = runGame(wins)
...
'''
def sendStatus(wins, level=0, state=False, score=0, lines=0, fallingPiece=None, nextPiece=None, board=[]):
    global NAME
    ...
    while True:
        ...
        if fallingPiece == None:
            ...
            if not isValidPosition(board, fallingPiece):
                status = sendStatus(wins, score=score, state=True,
                                    level=level, lines=clearLines)
                break
